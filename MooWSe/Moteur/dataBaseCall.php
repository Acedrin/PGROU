    ...
    function affichage($client_name) {
        ...
        while ($function_recup = $function_request->fetch()) {
            if ($function_recup != $last_function) {
                ...
                $variable_request = $base->query('SELECT variable.variable_name,type.type_name,variable.variable_input,variable.variable_order FROM variable INNER JOIN type ON type.type_id=variable.type_id
                ...
                while ($variable_recup = $variable_request->fetch()) {
                    ...
                    $variable_order=$variable_recup["variable_order"];
                    if ($variable_input == 1) {
                        echo'<div style="text-indent: 15px;">Argument(s) :</div>';
                    } else {
                        echo'<div style="text-indent: 15px;">Sortie :</div>';
                    }
                    echo '<div style="text-indent: 30px;">variable ' . $variable_name . ' de type ' . $type_name . ' ordre '.$variable_order.'</div>';
                }
                echo '</br>';
